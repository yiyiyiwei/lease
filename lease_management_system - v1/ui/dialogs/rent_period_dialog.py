...
logger = get_logger("RentPeriodDialog")
...
class RentPeriodDialog(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        self.parent = parent
        self.result: Optional[Tuple[datetime.date, datetime.date, float]] = None
    ...
    def _create_widgets(self):
        ...
        # 表单框架
        form_frame = ttk.LabelFrame(main_frame, text="租金期信息", padding="15")
        ...
        # 月租金
        ttk.Label(form_frame, text="月租金(元):").grid(row=2, column=0, sticky=tk.W, padx=5, pady=10)
        self.monthly_rent_var = tk.StringVar()
        monthly_rent_entry = ttk.Entry(form_frame, textvariable=self.monthly_rent_var, width=20)
        monthly_rent_entry.grid(row=2, column=1, sticky=tk.W, padx=5, pady=10)
        monthly_rent_entry.focus()  # 设置焦点
    ...
    def _validate_input(self) -> bool:
        ...
        try:
            ...
            # 验证月租金
            try:
                monthly_rent = float(self.monthly_rent_var.get())
                if monthly_rent <= 0:
                    self.message_var.set("月租金必须大于0")
                    return False
            except ValueError:
                self.message_var.set("月租金必须是有效数字")
                return False
    ...
    def _save(self):
        ...
        try:
            ...
            self.result = (start_date, end_date, monthly_rent)
            
            logger.info(f"租金期数据保存: {self.result}")
            self.destroy()
            
        except Exception as e:
            logger.error(f"保存租金期数据失败: {str(e)}")
